...
public class atalayaEIPTest extends CamelBlueprintTestSupport {
    ...
    public void transferCSV() throws Exception {
        ...
        Exchange exchange = template.request("netty4-http:http://localhost:8383/big/1.0/transfer/mysqlthedata",
                new Processor() {
                    public void process(Exchange exchange) throws Exception {
                        exchange.getIn().setHeader(Exchange.HTTP_METHOD, "POST");
                        exchange.getIn().setHeader(Exchange.CONTENT_TYPE, "text/plain;charset=UTF-8");
                        //exchange.getIn().setHeader("insertSQL", "insert into T100 (SPRSL, MSG, MSGNR, TEXT, BIG_TRANSFERDATE, BIG_TRANSFERTIME, BIG_TRANSFERTSTAMP) values (1, :#${body[MSG]}, 11, 11111, 11111, 11111, 11111)");
